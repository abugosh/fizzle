  ...
  (testing "Casting a modal card shows spell mode selection via standard selection system"
    (let [db (th/create-test-db)
      ...
      (let [selection (:game/pending-selection result-db)]
        ...
        (is (false? (:selection/auto-confirm? selection))
            "Should not auto-confirm (inline view dispatches explicitly)")
        (is (= 2 (count (:selection/choices selection)))
            "Should have choices for all candidates")
        (is (= "Choose a mode" (:selection/description selection))
            "Should have description for mode selection")
        (is (= (:selection/candidates selection) (:selection/valid-targets selection))
            "Valid targets should match candidates"))
