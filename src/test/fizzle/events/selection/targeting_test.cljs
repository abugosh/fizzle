...
(deftest ability-cast-targeting-executor-chaining-returns-db-unchanged
  (testing ":ability-cast-targeting executor chaining branch: chain is a no-op (returns db unchanged)"
    ;; :ability-cast-targeting is in the :targeting-to-mana-allocation hierarchy.
    ;; Chaining applies when the spell has BOTH targeting AND a generic mana cost.
    ;; Stifle has no generic mana cost (only {U}), so no real card produces chaining
    ;; ability-cast-targeting. We test this branch by building the selection directly
    ;; with :chaining lifecycle and confirming.
    ;; This is acceptable: the selection shape is well-specified and we enter via
    ;; sel-spec/set-pending-selection (the ADR-019 chokepoint).
    (let [db (setup-db {:mana {:blue 1}})
          ability-source-id (random-uuid)
          db-with-ability (stack/create-stack-item
                            db
          ...
          ability-eid (d/q '[:find ?e .
                           ...
                           db-with-ability)
          [db1 stifle-id] (th/add-card-to-zone db-with-ability :stifle :hand :player-1)
          stifle-obj (q/get-object db1 stifle-id)
          stifle-card (:object/card stifle-obj)
          ;; Build the selection directly with chaining lifecycle to test the chaining branch
          ;; The casting step builder (build-cast-time-target-selection) sets :chaining when
          ;; has-generic-mana-cost? is true. We simulate this with a constructed selection.
          chaining-sel {:selection/type :ability-cast-targeting
                        :selection/lifecycle :chaining
                        :selection/player-id :player-1
                        :selection/object-id stifle-id
                        :selection/mode {:mode/id :primary
                                         :mode/mana-cost {:blue 1}
                                         :mode/effects (:card/effects stifle-card)}
                        :selection/target-requirement {:target/id :ability
                                                       :target/type :ability
                                                       :target/required true}
                        :selection/valid-targets [ability-eid]
                        :selection/selected #{}
                        :selection/select-count 1
                        :selection/validation :exact
                        :selection/auto-confirm? true}
          ;; Entry via set-pending-selection (ADR-019 chokepoint)
          app-db (sel-spec/set-pending-selection {:game/db db1} chaining-sel)
          app-db' (update app-db :game/pending-selection assoc :selection/selected #{ability-eid})
          after-confirm (sel-core/confirm-selection-impl app-db')
          db' (:game/db after-confirm)]
      ;; Chaining branch: executor returns {:db game-db} unchanged — spell NOT cast yet
      ;; The chain builder (targeting-to-mana-allocation) should produce next selection
      ;; (mana-allocation). Since Stifle has no colorless cost, chain builder returns nil
      ;; and standard path triggers (cleanup). Stifle remains in hand.
      (is (= :hand (:object/zone (q/get-object db' stifle-id)))
          "Stifle should still be in hand — chaining executor returns db unchanged, spell not cast yet"))))
