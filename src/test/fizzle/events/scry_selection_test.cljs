...
(deftest test-build-scry-selection-large-scry
  ;; Bug caught: large scry amounts failing or returning wrong count
  (testing "Scry 5 on library with 10 cards returns all 5 correctly"
    (let [db (-> (init-game-state)
      ...
      (is (= expected-top-5 (:selection/cards result))
          "Scry 5 cards should match top 5 from library in order")
      (is (= [] (:selection/top-pile result))
